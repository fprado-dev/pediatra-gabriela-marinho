            ...
            return (
              <Reveal key={item.title} delay={index * .08}>
                <motion.article
                  ...
                  className="h-full flex flex-col gap-3.5 p-6 lg:p-7 rounded bg-card border border-card-line hover:border-brand hover:shadow-[var(--shadow)] transition-colors"
                >
                  <div className="flex items-baseline justify-between gap-4 pb-4 border-b border-line">
                    <h3 className="text-2xl lg:text-[1.75rem] font-bold text-ink">{item.title}</h3>
                    <Icon className="w-6 h-6 text-brand-deep shrink-0 self-center" />
                  </div>

                  <p className="text-[.9rem] text-muted">{item.description}</p>
