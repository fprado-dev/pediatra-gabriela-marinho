    ...
    <section id="atendimento" className="screen relative bg-ground bg-home-pattern">
      <div className="max-w-[1240px] w-full mx-auto px-5 sm:px-8 lg:px-20">
        <Reveal className="mb-4 lg:mb-13">
          ...
          <h2 className="text-[clamp(1.5rem,3.6vw,2.9rem)] font-bold leading-[1.12] text-ink mb-4 md:whitespace-nowrap">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 lg:gap-18 items-stretch">
            ...
            {specializedCare.differentials.map((differential, index) => {
              ...
              return (
                <div
                  ...
                  className="grid grid-cols-[auto_1fr] gap-3.5 md:gap-5 py-3 md:py-6 border-b border-line first:border-t first:border-line"
                >
                  <Icon className="w-5 h-5 md:w-6.5 md:h-6.5 text-coral mt-1 shrink-0" />
                  <div>
                    <h3 className="text-[.98rem] md:text-lg font-bold text-ink mb-1">{differential.title}</h3>
                    <p className="text-[.78rem] md:text-[.92rem] text-muted leading-relaxed">{differential.description}</p>
                  </div>
                </div>
              );
            })}
          ...
          <Reveal from="right" className="flex items-center">
            <figure className="relative w-full m-0 photo-arch-frame">
              <img
                ...
                className="relative z-[1] w-full max-h-[14vh] md:max-h-[44vh] object-cover object-[50%_30%] photo-arch"
                width={1067}
                height={1600}
              />
            </figure>
